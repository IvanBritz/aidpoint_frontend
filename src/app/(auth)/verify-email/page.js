...
import { CheckCircleIcon, ShieldCheckIcon, ArrowRightOnRectangleIcon, PaperAirplaneIcon } from '@heroicons/react/24/outline'

const Page = () => {
    const router = useRouter()
    const { logout, verifyEmailCode, resendEmailVerificationCode, user } = useAuth({
        middleware: 'auth',
    ...
    const submitForm = async (event) => {
        ...
        try {
            ...
            // Let the useAuth hook handle redirection after successful verification
        } catch (error) {
                ...
                <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 transform transition-all duration-300 hover:shadow-2xl">
                    ...
                    {(errors.code) && (
                        <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                            {errors.code && <p className="text-red-700 text-sm">{errors.code[0]}</p>}
                        </div>
                    )}
